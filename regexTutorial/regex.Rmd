...
#### [next](#phoneFixupIntro) {#regexUses}
## And what are regular expressions good for?
#### [next](#nameFixupIntro) {#phoneFixupIntro}
...
#### [next](#textMiningIntro) {#nameFixupIntro}
### fixing up names
...
#### [next](#wcAllCaps) {#textMiningIntro}

### text mining

#### [next](#wcHandles) {#wcAllCaps}
...
#### [next](#regexNamedClasses) {#regexEssentials}
...
#### [next](#regexFunctions) {#regexNamedClasses}
* **Combining & capturing:**
...
#### [next](#cleanPhones) {#regexFunctions}
## What can you do with **regular expressions**?
> **in the simple cases:** 

* ### **grep(** **regex**,**string**,value=FALSE,invert=FALSE)  [example](#grepEg) {#grep}
...
* ### **grepl(** **regex**,**string**)  [example](#greplEg) {#grepl}
...
* ### **sub(** **regex**,**replacement**,**string**)  [example](#subEg) {#sub}
    + #### returns a string where the first match of regex is changed to replacement
* ### **gsub(** **regex**,**replacement**,**string**)  [example](#gsubEg) {#gsub}
    + #### returns a string where all matches of regex are changed to replacement
* ### **strsplit(** **string**,**regex**)  [example](#strsplitEg) {#strsplit}
...
* ### **matchStart**<-**regexpr(** **regex**,**strings**)  [example](#regexprEg) {#regexpr}
...
#### [next](#cleaningNames) {#cleanPhones}
## Example:  Formatting Phone Numbers   [back](#back1) {#eg1}
...
```
#### [next](#nameCleaningSetUp) {#cleaningNames}
...
```
#### [next](#textMining)  {#nameCleaningSetUp}
...
```
#### [next](#twittersphere) {#tweetsDplyr}
...
* 
### **grep(** **regex**,**string**,value=FALSE,invert=FALSE)  [back](#grep) {#grepEg}
* 
    + #### grep is an acronym for "get regular expression"
    + #### with the default arguments, it returns the indices of string that match regex
    + #### with invert=TRUE, it returns the indices of string that don't match regex
...
### **grepl(** **regex**,**string**)  [back](#grepl) {#greplEg}
* 
    + #### grep logical, returns a logical vector TRUE wherever string matches regex
    + #### !grepl returns a logical vector TRUE wherever string does not match regex
    + #### grepl is particularly useful in filtering a dplyr pipeline
...
### **sub(** **regex**,**replacement**,**string**)  [back](#sub) {#subEg}
* 
    + #### returns a string where the first match of regex is changed to replacement
...
### **gsub(** **regex**,**replacement**,**string**)  [back](#gsub) {#gsubEg}
* 
    + #### returns a string where all matches of regex are changed to replacement
...
### **strsplit(** **string**,**regex**)  [back](#strsplit) {#strsplitEg}
* 
    + #### returns a list the length of string. 
    + #### each element of the list contains a vector,
    + #### whose elements are formed by dividing string at the matches of regex
...
### **matchStart**<-**regexpr(** **regex**,**strings**)  [back](#regexpr) {#regexprEg}
* 
    + #### **matchStart** is a vector the length of strings
    + #### whose each element is the index of the first match in the corresponding element of strings,
    + #### **attr(matchStart,"match.length")** contains the length of the first match in the corresponding element of strings,
